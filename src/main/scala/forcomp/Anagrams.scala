...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val occurrences = for {
      (c, l) <- w.groupBy(c => c.toLower)
    } yield (c, l.length())
    occurrences.toList.sortBy(p => p._1)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    val rest = for {
      a <- x
      if y.forall(p => p._1 != a._1)
    } yield a

    (subtracted ::: rest).sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsHelper(occurrences: Occurrences): List[Sentence] = {
      if (occurrences == Nil) List(Nil)
        ...
        valid_words <- dictionaryByOccurrences.getOrElse(subset, Nil)
        rest <- sentenceAnagramsHelper(subtract(occurrences, subset))
      } yield valid_words :: rest
    }
